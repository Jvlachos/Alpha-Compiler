...
bool check_self(avm_table* table,avm_table* element){
...
string print_table(avm_table* table){
    ...
    for (str = table->strIndexed.begin(); str != table->strIndexed.end(); ++str) {
        if(str->second->type == table_m && check_self(table,str->second->data.tableVal)){
    ...
    for (num = table->numIndexed.begin(); num != table->numIndexed.end(); ++num) {
         if(num->second->type == table_m && check_self(table,num->second->data.tableVal)){
    ...
    for (func = table->userfuncIndexed.begin(); func != table->userfuncIndexed.end(); ++func) {
         if(func->second->type == table_m && check_self(table,func->second->data.tableVal)){
              cout << "{KEY:" << func->first << ", VALUE:" << func->second<<"}"
             << " ";
             break;
        }
    ...
    for (l_func = table->libFuncIndexed.begin(); l_func != table->libFuncIndexed.end(); ++l_func) {
         if(l_func->second->type == table_m && check_self(table,l_func->second->data.tableVal)){
              cout << "{KEY:" << l_func->first << ", VALUE:" << l_func->second<<"}"
             << " ";
             break;
        }
    ...
    for (tab = table->tableIndexed.begin(); tab != table->tableIndexed.end(); ++tab) {
         ...
         if( check_self(table,tab->second->data.tableVal) || check_self(table,tab->first) || check_self(tab->first,tab->second->data.tableVal)){
              cout << "{KEY:" << tab->first << ", VALUE:" << tab->second<<"}"
             << " ";
             break;
        }
    ...
    for (b_ool = table->boolIndexed.begin(); b_ool != table->boolIndexed.end(); ++b_ool) {
         if(str->second->type == table_m && check_self(table,b_ool->second->data.tableVal)){
              cout << "{KEY:" << b_ool->first << ", VALUE:" <<b_ool->second<<"}"
             << " ";
             break;
        }
